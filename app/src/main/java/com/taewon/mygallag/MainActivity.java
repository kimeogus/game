...
public class MainActivity extends AppCompatActivity {
    ...
    private void init(){
        Display display = getWindowManager().getDefaultDisplay();//view 의 display얻어오기
        ...
        //음악바꾸기
        changeBgMusic();
        bgMusic.setOnCompletionListener(new MediaPlayer.OnCompletionListener() {//음악끝나면 음악바꾸기
            @Override
        ...
        bulletCount.setText(spaceInvadersView.getPlayer().getBulletsCount() + "/30");//30/30
        scoreTv.setText(Integer.toString(spaceInvadersView.getScore()));//score:0
        ...
        bgMusic.start();//음악 바뀌면서 재생
    }

    private void changeBgMusic(){
        bgMusic = MediaPlayer.create(this,bgMusicList.get(bgMusicIndex));
        bgMusic.start();
        bgMusicIndex++;//증가하면 음악 바뀜
        bgMusicIndex = bgMusicIndex%bgMusicList.size();//음악 개수만큼만 바뀌게 하기위해
    }
