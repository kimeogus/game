...
public class SpaceInvadersView extends SurfaceView implements Runnable, SurfaceHolder.Callback {
    ...
    public SpaceInvadersView(Context context, int characterId,int x, int y){
        ...
        ourHolder = getHolder();//현재 SurfaceView를 리턴받기
        paint = new Paint();
        screenW = x;
        screenH = y;//받아온x,y
        src = new Rect();//원본 사각형
        dst = new Rect();//사본 사각형
        dst.set(0,0,screenW,screenH);//시작x,y와 끝 x,y
        startGame();
    }
    ...
    private void initSprites(){
        starship = new StarshipSprite(context,this,characterId,screenW/2,screenH-400,1.5f);//starshipsprite생성 아이템들 생성
    ...
    public void run() {
        while (running){
            Random r = new Random();
            boolean isEnemySpawn = r.nextInt(100)+1<(getPlayer().speed+(int)(getPlayer().getPowerLevel()/2));// 1~100의 랜덤 수가 player의 스피드+player의 powerlever/2보다 작을 때 실행
